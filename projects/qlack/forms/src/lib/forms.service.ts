...
export class QFormsService {
  ...
  /**
   * Converts a data object into a query string to be used together with Spring Data for
   * data-filtering.
   * @param data The data object to convert, for example FormGroup::value or FormGroup::getRawValue.
   * @param aliases An array of aliases to change the name of the FormGroup elements to
   * different names in the resulting query string. For example, you may have a FormGroup element
   * named 'session' whereas in your model you have a 'sessionId' column; you can use an alias such
   * as `new FilterAliasDTO('session', 'sessionId')` and this method will produce a query string
   ...
   *
   */
  dataToQueryString(data: any, aliases: QFilterAlias[], includeEmpty: boolean): string {
    let query = '';
    for (const filter in data) {
      // Skip empty fields if requested to not include them.
      if ((data[filter] == null || data[filter] === '') && !includeEmpty) {
        continue;
      }
      ...
      if (data[filter]._isAMomentObject) {
        value = data[filter].toISOString();
      } else {
        value = data[filter];
      }
   ...
   * @param sortDirection
   */
  makeQueryStringForData(data: any, aliases: QFilterAlias[], includeEmpty: boolean,
                  page: number, size: number, sort: string, sortDirection: string): string {
    return this.appendPagingToFilter(this.dataToQueryString(data, aliases, includeEmpty), page,
      size, sort, sortDirection);
  }
  ...
  /**
   * Cleans up an object of null/undefined values. For example:
   *
   * fb: FormGroup;
   * ...
   * myService.save(cleanupData(this.fb.getRawValue()) as MyServiceDto)
   *
   * @param formData
   */
  cleanupData(formData: any): Partial<any> {
